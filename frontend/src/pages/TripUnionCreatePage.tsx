...
export default function TripUnionCreatePage() {
  const { token } = useAuth();
  ...
  const [transportsTotalPages, setTransportsTotalPages] = useState(1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        "http://localhost:8080/trip-union",
        {
      ...
      setError(error?.response?.data?.message || "Ошибка создания путевого листа");
    } finally {
      setLoading(false);
    }
  };

  // Сброс формы для создания нового путевого листа
  const handleCreateAnother = () => {
    setSelectedRoute(null);
    setSelectedDriver(null);
    setSelectedTransport(null);
    setSelectedTrailer(null);
    setSelectedFirmCustomer(null);
    setSelectedFirmCarrier(null);
    setCosts("");
    setDescription("");
  ...
  return (
    <div className="trip-create-root">
      <div className="create-header-row">
        <h2 className="create-title">Создание путевого листа</h2>
        <button
          type="button"
      ...
      {showSuccess && (
        <div className="modal-success">
          <div className="modal-success-content">
            <h3>Путевой лист успешно создан!</h3>
            <div className="modal-success-actions">
              <button onClick={() => navigate('/trip-union')}>Вернуться к списку</button>
              <button onClick={handleCreateAnother}>Создать ещё</button>
            </div>
          </div>
        </div>
      )}
